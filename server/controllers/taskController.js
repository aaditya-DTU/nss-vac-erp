  ...
  try {
    ...
    await Promise.all(
      audience.map((studentId) => {
        if (io) {
          io.to(`user:${studentId}`).emit("task:new", {
            ...task.toObject(),
            mySubmission: null,
          });
        }
  ...
  try {
    ...
    if (req.user.role === "student") {
      ...
      query.$or = [
        ...
        {
          "assignedTo.scope": "specific",
          "assignedTo.studentIds": req.user._id,
        },
        {
          ...
          $and: [
            {
              $or: [
                { "assignedTo.filter.year": { $exists: false } },
                { "assignedTo.filter.year": req.user.year },
              ],
            },
            {
              $or: [
                { "assignedTo.filter.branch": { $exists: false } },
                { "assignedTo.filter.branch": req.user.branch },
              ],
            },
            {
              $or: [
                { "assignedTo.filter.section": { $exists: false } },
                { "assignedTo.filter.section": req.user.section },
              ],
            },
          ],
        },
...
exports.updateTask = async (req, res, next) => {
  try {
    ...
    if (io) {
      const audience = await resolveAudience(task);
      audience.forEach((studentId) => {
        io.to(`user:${studentId}`).emit("task:updated", task);
      });
    }
...
exports.deleteTask = async (req, res, next) => {
  try {
    ...
    if (io) {
      const audience = await resolveAudience(task);
      audience.forEach((studentId) => {
        io.to(`user:${studentId}`).emit("task:archived", { _id: task._id });
      });
    }
