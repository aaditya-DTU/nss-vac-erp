...
export default function Tasks() {
  ...
  useEffect(() => {
    ...
    const onNewTask = (task) => {
      ...
      toast(`📌 New task: ${task.title}`, { duration: 5000 });
    };

    const onUpdatedTask = (task) => {
      setTasks((prev) =>
        prev.map((t) =>
          t._id === task._id ? { ...task, mySubmission: t.mySubmission } : t,
        ),
      );
      toast(`✏️ Task updated: ${task.title}`, { duration: 4000 });
    };

    const onArchivedTask = ({ _id }) => {
      setTasks((prev) => prev.filter((t) => t._id !== _id));
    };

    socket.on("task:new", onNewTask);
    socket.on("task:updated", onUpdatedTask);
    socket.on("task:archived", onArchivedTask);

    return () => {
      socket.off("task:new", onNewTask);
      socket.off("task:updated", onUpdatedTask);
      socket.off("task:archived", onArchivedTask);
    };
  }, [socket]);
  ...
  const ledgerItems = pastTasks
    ...
    .map((t) => {
      ...
      return {
        ...
        badgeClass:
          status === "none" ? "bg-red-100 text-red-700" : statusStyles[status],
      };
    });
    ...
    <>
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
        {liveTasks.map((t) => {
          const status = t.mySubmission?.status || "none";
          const overdue =
            new Date(t.deadline) < new Date() && status === "none";
          return (
            <Link
              to={`/tasks/${t._id}`}
              key={t._id}
        ...
        {liveTasks.length === 0 && (
          <p className="text-ink/50">No active tasks assigned.</p>
        )}
      </div>
      ...
      <Ledger
        title="Past tasks"
        items={ledgerItems}
        onItemClick={openTaskLedgerSummary}
      />
      ...
      <LedgerSummaryModal
        ...
        subtitle={
          ledgerData?.task
            ? `Deadline was ${format(new Date(ledgerData.task.deadline), "MMM d, yyyy")}`
            : ""
        }
        stats={
          ledgerData?.task
        ...
        rows={
          ledgerData?.task
            ? [
                {
                  id: "me",
                  primary: ledgerData.task.mySubmission
                    ? "Your submission"
                    : "No submission made",
                  secondary:
                  ...
                  note: ledgerData.task.mySubmission?.reviewNote
                    ...
                    : undefined,
                  badge: (
                    ledgerData.task.mySubmission?.status || "missed"
                  ).replace("_", " "),
                  badgeClass:
                    statusStyles[ledgerData.task.mySubmission?.status] ||
